 ...
 */
public class BusboyAccountController implements Initializable {
    private static final Logger logger = Logger.getLogger(BusboyAccountController.class.getName());
  ...
  @FXML
    void onBack(ActionEvent event) {
        ...
        // Load the kitchen view
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/Views/busboyViews/busboyTables.fxml"));
        Parent root = loader.load();
        
        // Get the controller and pass the current user
        Controllers.Busboy.BusboyTableController controller = loader.getController();
        if (controller != null && currentUser != null) {
            controller.setCurrentUser(currentUser);
            System.out.println("Set current user in Buysboy when returning: " + currentUser.getUserId());
        } else {
            System.out.println("Warning: " + 
                (controller == null ? "Controller is null" : "CurrentUser is null") + 
                " when returning to Busboy view");
        }
        
        // Set up the scene
        Scene scene = new Scene(root, 1200, 800); // Use appropriate dimensions
        currentStage.setScene(scene);
        currentStage.setTitle("Busboy View - " + (currentUser != null ? currentUser.getFullName() : ""));
        
        logger.info("Returned to Busboy view");
    } catch (IOException ex) {
        System.out.println("Error returning to Busboy view: " + ex.getMessage());
        ex.printStackTrace();
        logger.log(Level.SEVERE, "Error returning to Busboy view", ex);
        
        // Show error alert
        Alert alert = new Alert(AlertType.ERROR);
        alert.setTitle("Navigation Error");
        alert.setHeaderText("Could not return to kitchen view");
