...
public class AccountController implements Initializable {
    ...
    void onBack(ActionEvent event) {
        ...
        Controllers.cook.CookViewController controller = loader.getController();
        if (controller != null && currentUser != null) {
            controller.setCurrentUser(currentUser);
            System.out.println("Set current user in KitchenViewController when returning: " + currentUser.getUserId());
        } else {
            System.out.println("Warning: " + 
                (controller == null ? "Controller is null" : "CurrentUser is null") + 
                " when returning to kitchen view");
        }
        
        // Set up the scene
        Scene scene = new Scene(root, 1200, 800); // Use appropriate dimensions
        currentStage.setScene(scene);
        currentStage.setTitle("Kitchen View - " + (currentUser != null ? currentUser.getFullName() : ""));
        
        logger.info("Returned to kitchen view");
    } catch (IOException ex) {
        System.out.println("Error returning to kitchen view: " + ex.getMessage());
        ex.printStackTrace();
        logger.log(Level.SEVERE, "Error returning to kitchen view", ex);
