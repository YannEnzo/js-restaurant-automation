...
public class manager_floorController implements Initializable, TableStatusObserver {
    private static final Logger logger = Logger.getLogger(manager_floorController.class.getName());
    ...
    private void handleTableClick(String tableNumber) {
        try {
            // Get the table from the database by table number
    ...
    private void showTableInfo(RestaurantTable table) {
        ...
        if (result.isPresent() && result.get() == changeStatusButton) {
            // Make sure we have a current user before attempting to change status
            if (currentUser != null) {
                showChangeStatusDialog(table);
            } else {
                logger.severe("Cannot change table status: Current user is null");
                showAlert(AlertType.ERROR, "Error", "Cannot change table status", 
    ...
    private void showChangeStatusDialog(RestaurantTable table) {
        ...
        result.ifPresent(newStatus -> {
            try {
                // Only update if status has changed and current user is not null
                if (!newStatus.equals(table.getStatus()) && currentUser != null) {
    ...
    @FXML void onontableC5(ActionEvent event) { handleTableClick("C5"); } // Fixed method name
